...
class BusinessFinancialPersonalcostController extends GetxController {
  final FirebaseRemoteConfig _remoteConfig = FirebaseRemoteConfig.instance;
  var business_survey_questions_personal = <Map<String, dynamic>>[].obs;
  ...
  void loadCachedQuestions() async {
    ...
    if (cachedQuestions != null) {
      business_survey_questions_personal.value =
          await List<Map<String, dynamic>>.from(
              jsonDecode(cachedQuestions)['questions']);
      print(
          'loaded cached questions:${business_survey_questions_personal.value}');
    }
  }
  ...
  Future<void> fetchSurveyQuestions() async {
    try {
      ...
      String questionsData =
          _remoteConfig.getString('business_financial_personal_key');

      print('Raw questions data from Remote Config: $questionsData');

      if (questionsData.isNotEmpty) {
        var decodedData = jsonDecode(questionsData);
        if (decodedData['business_financial_personal_key'] != null) {
          business_survey_questions_personal.value =
              List<Map<String, dynamic>>.from(
                  decodedData['business_financial_personal_key']);
          print(
              'Parsed questions: ${business_survey_questions_personal.value}');
